...
> 사용자의 과거 패턴을 스스로 학습하는 **개인 지식 그래프(PKG)** 기반 카페인 인과관계 추적 및 AI 코칭 시스템
...
현재 **Phase 1(기반 구축)**을 완료하고, **Phase 2(AI 엔진 통합)** 단계에 있습니다.
...
- [ ] **개인 온톨로지 자동 생성**: LLM을 활용해 사용자 고유의 부작용 패턴(`콜라` -> `두통`)을 스스로 학습하는 기능
- [ ] **GraphRAG 엔진 개발**: 의학 지식(RAG)과 개인 패턴(Graph)을 결합한 다중 홉 추론 엔진 구현
...
### 1. 🧠 AI 카페인 탐정 (GraphRAG Engine)
*   **개인 온톨로지 자동 생성**: 사용자의 기록이 쌓이면 AI가 스스로 `[특정 음료] -> [특정 증상]` 간의 인과관계를 발견하여 Neo4j 지식 그래프에 개인화된 규칙으로 등록합니다.
*   **추론 기반 AI 챗봇**: "왜 오늘 눈이 아프지?"라는 질문에 "과거 데이터 분석 결과, 어제 마신 콜라의 리바운드 효과일 확률이 85%입니다"라고 논리적으로 답변합니다.
...
사용자의 개인별 대사 가중치($\alpha$)를 반영한 지수적 감쇄 공식을 사용합니다.
$$C_{now} = C_{initial} \times 0.5^{\frac{t}{halfLife \times \alpha}}$$

*   **$\alpha$ (가중치)**: 생리 중(1.5), 초민감자(1.2~1.8), 고내성자(0.8) 등 개인별 동적 변수

### 2. GraphRAG 인과관계 추론
Neo4j의 사이퍼(Cypher) 쿼리를 통해 특정 섭취 사건과 증상 사이의 최단 경로 및 반복 패턴을 탐색합니다.
...
- **AI Engine**: LangChain, GraphRAG, GPT-4o
